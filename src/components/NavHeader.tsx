    ...
    <header className="sticky top-0 bg-[#FBFBFA]/80 backdrop-blur-md z-40 border-b border-slate-100/60">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-5 text-sm text-slate-500">
          ...
          {lastUpdated && (
            <span className="hidden sm:inline border-l border-slate-200 pl-5">更新: {formatBeijingTime(lastUpdated)}</span>
          )}
          <button
            className="sm:hidden p-1.5 -mr-1.5 text-slate-500 hover:text-slate-700"
      ...
      {mobileOpen && (
        <div className="sm:hidden border-t border-slate-100/60 bg-[#FBFBFA]/95 backdrop-blur-md">
          <nav className="max-w-7xl mx-auto px-6 py-3 flex flex-col space-y-1">
            {NAV_LINKS.map((link) => {
              const isActive = pathname === link.href;
              return (
                <Link
            ...
            {lastUpdated && (
              <div className="px-4 pt-2 pb-1 text-xs text-slate-400 border-t border-slate-100 mt-1">
                更新: {formatBeijingTime(lastUpdated)}
              </div>
            )}
          </nav>
        </div>
